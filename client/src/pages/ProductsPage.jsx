...
const ProductsPage = () => {
  ...
  useEffect(() => {
    // Skip during initial load since we already fetch products in the first effect
    ...
    const delayDebounce = setTimeout(() => {
      ...
      // No need to update dataLoaded again since this is for filtering only
      dispatch(getProducts(params));
    }, 500);

    return () => clearTimeout(delayDebounce);
  }, [dispatch, filters.category, filters.search, isInitialLoad]);
  ...
  // Only show loader on initial page load, never on filter changes
  if (isInitialLoad && (loading || categoryLoading)) {
    return <Loader />;
  }
