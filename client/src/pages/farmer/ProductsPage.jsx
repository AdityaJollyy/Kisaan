...
const ProductsPage = () => {
  ...
  // Filter products only when searchTerm or farmerProducts changes
  useEffect(() => {
    if (Array.isArray(farmerProducts)) {
      setFilteredProducts(
  ...
  if (loading && !dataFetched && farmerProducts.length === 0) {
    return <Loader />;
  }
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredProducts.map((product) => (
                  <tr key={product._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div
                        className={`text-sm font-medium ${product.quantityAvailable === 0
                          ? "text-red-500"
